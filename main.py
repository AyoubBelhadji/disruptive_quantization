...
if __name__ == "__main__":
    ...
    config_folder = os.path.join(os.path.dirname(__file__), 'experiment_configs')
    ...
    for config_filename in os.listdir(config_folder):
        if config_filename.endswith('.json'):  # Ensure it's a JSON file
            ...
            try:
                ...
                print(f"Loaded dataset shape for {config_filename}: {data.shape}")
            ...
            try:
                ...
                experiment_full_id = sim_manager.save_experiments(
                    experiment_name=config_filename.split('.')[0],  # Use the filename (without extension) as the experiment name
                ...
                if show_gif_visualization:
                    visualize_and_save_dynamics(experiment_full_id,rand_algo.c_array_trajectory, rand_algo.data_array)
