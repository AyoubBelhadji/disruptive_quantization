...
def create_lastiteration_fig(data_array, centroids, r, alg_name, subpath, **ax_kwargs):
    T = centroids.shape[0]
    N_data = data_array.shape[0]
    alpha_data = min(0.5, max(15/N_data,1))
    fig, ax = plt.subplots()
    ...
    ax.set_title(f"Centroid dynamics, t={T-1}, {alg_name}")
    centroids_t_scatter.set_offsets(centroids[T-1])
    ...
    fig_path = os.path.join(
        folder_name, f"particle_evolution_{r}" + '.pdf')
    # writer_classes = [FFMpegWriter, ImageMagickWriter, PillowWriter]
    # for writer_class in writer_classes:
    #     try:
    #         writer = writer_class(fps=10)
...
def centroid_dynamics(alg_name, y_trajectory, data_array, subpath, file_format="mp4", limit_margin=0.2):
    ...
    for r in range(R):
        ...
        create_lastiteration_fig(data_array, centroids_r, r, alg_name, subpath, xlim=xlims, ylim=ylims)